...
- Repetition dominated: This is the simplest explainable replay pattern. The main evidence is repeated query/domain behaviour with confirmed query repetition, without needing burst or broader context to carry the decision. The example event is `evt_009777`, which clicked `www.overstock.com` for query `fuzz sulphid`.
