...
The combined score uses a 0.58/0.42 heuristic/ML split because the rules layer is more directly explainable and should remain slightly dominant, while ML still has enough weight to move borderline cases and catch multivariate oddities. The thresholds are conservative guardrails, not learned cutoffs. The same 0.62 heuristic and 0.995 EIF rank-tail agreement thresholds used in suppression are also reported separately so blind spots are visible. The EIF agreement threshold is intentionally narrow: this run reports {ml_only_count:,} `ML only` events in the extreme anomaly tail, while broad top-decile anomaly ranks previously created too many standalone ML entries without enough explainable support. The method-disagreement table is diagnostic; it does not expose another production model path.
