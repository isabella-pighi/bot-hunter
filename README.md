...
The exact time-to-click reuse rule is the only heuristic that uses percentile calibration. It counts how often each non-negative `ttc` value appears, uses the nearest-rank 99th percentile of those reuse counts, and applies an absolute floor of `40`; the effective cutoff is `max(40, p99_count)`. This rule is calibrated because exact `ttc` reuse is a global duplicate-count signal whose suspiciousness depends on the dataset's timer granularity and reuse distribution. The floor keeps small or unusually smooth datasets from treating low-count coincidences as bot evidence. Other heuristic cutoffs remain fixed or total-rate based because they represent separate behavioral concepts and were not part of this selective calibration.
...
The same rule hits are also stored as structured `rule_contributions` in `artifacts/sample_events.json`. Each contribution has a stable `rule_id`, display `label`, compatibility `reason`, numeric `weight`, raw `observed` value, threshold where applicable, `threshold_mode`, and the condition that fired. Fixed-threshold rules report `threshold_mode: "absolute"`; adaptive exact-ttc reuse reports `threshold_mode: "adaptive_percentile"` when it appears in sampled events or downstream rule-contribution exports. Keeping both forms matters for explainability: business users can still read concise reasons, while audits, dashboards, grouped analysis, and per-rule impact checks can rely on stable machine-readable fields instead of parsing English text.
